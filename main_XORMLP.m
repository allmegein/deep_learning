...
for i = 1 : trainPar.it
    for j = 1 : trainPar.noi
        ...
        delta3_1 = y_hat(j) * (1 - y_hat(j)) * (trainPar.y(j) - y_hat(j));  % neuron's delta value at output layer
        delta2_1 = X2(2) * (1 - X2(2)) * w(3,2) * delta3_1;                 % first neuron's delta value at hidden layer
        delta2_2 = X2(3) * (1 - X2(3)) * w(3,3) * delta3_1;                 % second neeuron's delta value at hidden layer

        % Update the unknown parameters
        w(1,:) = w(1,:) + mu * trainPar.x(j,:) * delta2_1;
        w(2,:) = w(2,:) + mu * trainPar.x(j,:) * delta2_2;
        w(3,:) = w(3,:) + mu * X2 * delta3_1;
